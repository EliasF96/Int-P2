...
const ProductCardCart = ({product})=>{
    ...
    const [productBuy, setProductBuy] = useState([])
        const addProductToCart = async(id)=>{ 
            try{
                const url= `https://65399380e3b530c8d9e885df.mockapi.io/bakery/products/${id}`
                const response = await fetch(url)
                const data = await response.json()
                data.amount = amount //SET A AMOUNT OF ITEMS
                let confirm = window.confirm("Confirm Buy this product?");
                if(confirm){
                    fetch(`http://localhost:3000/buyList`, {
                            method: "POST",
                        ...
                        .then(toast.success("Purchase successfully"))
                        /*setInterval(() => {
            ...
            <>
                <Card className='productCard' >
                    <Card.Body>
                        ...
                        <Card.Text> <strong>Made in: </strong>{product.ageSince}</Card.Text>
                        <Card.Text> <strong>Valid Until: </strong>{product.ageUntil}</Card.Text>
                        ...
                        <Button variant="outline-success" onClick={()=> addProductToCart(product.id) }>Buy</Button>
                    </Card.Body>
                </Card>
                <ToastContainer/>
            </>
