...
export const ProductForm = ({ fetchData }) => {
  ...
  const [activate, setActivate] = useState(Boolean) //TO OPEN FORM MODAL

  const post = async (event) => {

    //CHECK THE WHOLE FORM BEFORE SEND THE COMMENTARY 
    // if (!form.checkValidity()) {
    //   toast.error("Must comple the whole form to send your commentary")
    //   for (let i = 0; i < form.length - 1; i++) {
    //     if (form[i].checkValidity() == false) {
    //       toast.warning(`Complete the input: ${form[i].id}`)
    //     }
    //   }
    // } else {
    ...
    if (!formCampValidation.name) {
      toast.error(`Complete the name Camp!`)
    } else if (!formCampValidation.price) {
      toast.error(`Complete the surname camp!`)
    } else if (!formCampValidation.stock) {
      toast.error(`Complete the stock camp`)
    } else if (!formCampValidation.mark) {
      toast.error(`Complete the mark camp`)
    } else if (!formCampValidation.category) {
      toast.error(`Complete the category  camp`)
    } else if (!formCampValidation.descShort) {
      toast.error(`Complete the descShort camp`)
    } else if (!formCampValidation.descLarge) {
      toast.error(`Complete the descLarge  camp`)
    } else if (!formCampValidation.freeDelivery) {
      toast.error(`Complete the freeDelivery  camp`)
    } else if (!formCampValidation.ageSince) {
      toast.error(`Complete the ageSince  camp`)
    } else if (!formCampValidation.ageUntil) {
      toast.error(`Complete the ageUntil  camp`)
    } else if (!formCampValidation.image) {
      toast.error(`Complete the image camp`)
    } else {
      toast.success("Form completed successfully!")

      let answer = window.confirm("Are you sure want to add this product?");
      if (answer) {
        fetch(`https://65399380e3b530c8d9e885df.mockapi.io/bakery/products`, {
          method: "POST",
